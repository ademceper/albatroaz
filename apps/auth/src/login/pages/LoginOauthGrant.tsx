            ...
            headerNode={
                <>
                    {client.attributes.logoUri && (
                        <img src={client.attributes.logoUri} alt="" className="mx-auto mb-2 max-h-12" />
                    )}
                    {msg("oauthGrantTitle")} <strong>{advancedMsg(client.name ?? client.clientId)}</strong>
                </>
            }
        >
            <div id="kc-oauth" className="space-y-3">
                ...
                <ul className="space-y-1 text-xs">
                    {oauth.clientScopesRequested.map((clientScope) => (
                        <li key={clientScope.consentScreenText} className="rounded border p-2">
            ...
            <form className="flex gap-2" action={url.oauthAction} method="POST">
                ...
                <Button type="submit" variant="outline" size="lg" name="cancel" className="flex-1">
                    {msgStr("doDecline")}
                </Button>
